...
| [`virtio-net`](https://github.com/cloud-boot/virtio-net) · [`snp`](https://github.com/cloud-boot/snp) | **Empty placeholders.** Each holds one Renovate config and no code. The work each was named for exists, elsewhere: the virtio-net driver is [`go-virtio/net`](https://github.com/go-virtio/net), a standalone pure-Go implementation of the Virtio 1.0+ init sequence and split-virtqueue TX/RX path, which `tamago-uefi` imports at v0.1.1 and wraps in a UEFI-side transport (`uefiboard/virtio_net_uefi.go`); SNP here means the UEFI **Simple Network Protocol**, and that one is inside [`tamago-uefi`](https://github.com/cloud-boot/tamago-uefi) (`uefiboard/simple_network_protocol.go`). Listed rather than quietly dropped, because the names are referenced elsewhere and a reader who follows one deserves to know where the code is. |
